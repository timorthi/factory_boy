...
class RelatedListFactoryTestCase(unittest.TestCase):
    ...
    def test_related_factory_list_of_lazily_evaluated_size(self):
        class TestRelatedObject:
            def __init__(self, obj=None, one=None, two=None):
                # Mock out the 'List of Related Objects' generated by RelatedFactoryList
                if hasattr(obj, 'related_list'):
        ...
        class TestObjectSelfAttrFactory(factory.Factory):
            class Meta:
                model = TestObject
            one = factory.Faker('pyint', min_value=1, max_value=10)
            two = 2
            three = factory.RelatedFactoryList(TestRelatedObjectFactoryList, 'obj',
                                               size=factory.SelfAttribute('one'))

        class TestObjectLazyAttrFactory(factory.Factory):
            class Meta:
                model = TestObject
            one = 1
            two = factory.Faker('pyint', min_value=1, max_value=10)
            three = factory.RelatedFactoryList(TestRelatedObjectFactoryList, 'obj',
                                               size=factory.LazyAttribute(lambda a: a.two * 3))

        self_attr_obj = TestObjectSelfAttrFactory.build()
        self.assertEqual(len(self_attr_obj.related_list), self_attr_obj.one)


        lazy_attr_obj = TestObjectLazyAttrFactory.build()
        self.assertEqual(len(lazy_attr_obj.related_list), lazy_attr_obj.two * 3)
